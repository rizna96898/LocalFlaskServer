...
class MemoryManager:
    ...
    def update_memory(self, body: Dict, session_id: str, last_user_content: str, last_assistant_content: str):
        print(f"[MEMORY] session_id={session_id} → 記憶更新を開始")
        self._run_memory_async(body, session_id, "update", last_user_content, last_assistant_content)

    def _extract_character_context(self, body: Dict) -> str:
        parts = [
    ...
    def _extract_relationship_names(self, world_relationships: list[str]) -> list[str]:
        names: list[str] = []
    ...
    def _run_memory_async(self, body: Dict, session_id: str, operation: str, user: str, char: str):
        def task():
            try:
                ...
                if operation == "create":
                    char_info = self._extract_character_context(body)
                    prompt_messages = self.prompt_builder.create_memory_prompt(char_info)
                else:
                    world_memory = file_utils.load_yaml_file(config.SESSIONS_DIR / session_id / "world_memory.yaml") or {}
                    char_info = self._extract_character_context(body)
                    prompt_messages = self.prompt_builder.update_memory_prompt(char_info, user, char, world_memory)
                ...
                #print("[DEBUG RAW REPR]")                
                #print("BEFORE:", repr(response_text))
                response_text = string_utils.strip_code_block(response_text)
                #print("AFTER :", repr(response_text))
                ...
                #print(f"[MEMORY] 生レスポンス保存: {temp_file.name}")
                ...
                #print(f"[WORLD] world_relationships: {world_relationships}")
                ...
                #print("[DEBUG normalized_memory before save]")
                #print(repr(normalized_memory))
                ...
                #print(f"[WORLD] character sync call start: {relation_names}")
                self._sync_session_character_files(session_id, relation_names)
                #print(f"[WORLD] character sync call end")

                #print(f"[CHAR MEMORY] async create start: {relation_names}")
                #description = body.get("description", "")
                #print(f"PERSONARITY: {description}")
                ...
                file_utils.save_yaml_file(config.SESSIONS_DIR / session_id / "world_memory.yaml", error_memory)
